import pytest
import os
from llm_inference_engine import AzureOpenAIInferenceEngine, BasicLLMConfig, OpenAIReasoningLLMConfig
...
class TestAzureOpenAIEngine:
    
    @pytest.fixture
    def gpt_engine(self):
        config = BasicLLMConfig(max_new_tokens=1024, temperature=0.5)
        return AzureOpenAIInferenceEngine(model=MODEL_GPT4, config=config)

    def test_azure_openai_chat_sync(self, gpt_engine):
        messages = [{"role": "user", "content": "Hello OpenAI"}]
        response = gpt_engine.chat(messages)
        print(f"\n[Sync] Response: {response['response']}")
        assert len(response["response"]) > 0

    def test_azure_openai_chat_stream(self, gpt_engine):
    ...
    @pytest.mark.asyncio
    async def test_azure_openai_chat_async(self, gpt_engine):
    ...
    @pytest.mark.asyncio
    async def test_azure_openai_reasoning_model(self):
        ...
        engine = AzureOpenAIInferenceEngine(model=MODEL_REASONING, config=config)
        ...
        try:
            ...
            pytest.fail(f"Azure OpenAI Reasoning model failed: {e}")